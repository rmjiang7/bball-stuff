...
def download_season_data(player_id):
    if os.path.exists("../data/players_data/%s" % player_id):
        return

    pathlib.Path("../data/players_data/%s" % player_id).mkdir(parents = True, exist_ok = True)
    ...
    for season in Seasons:
        ...
        # write to csv
        if df.shape[0] > 0:
            df.to_csv('../data/players_data/%s/%s_shots.csv' % (player_id,season) , sep = ',', index = False)
