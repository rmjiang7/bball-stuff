...
pathlib.Path("pt_data").mkdir(exist_ok = True)

# Download each measure type, for each season
PtMeasureTypes = ['Drives', 'Defense', 'CatchShoot', 'Passing', 'Possessions', 'PullUpShot', 'Rebounding', 'Efficiency', 'SpeedDistance', 'ElbowTouch', 'PostTouch', 'PaintTouch']
...
for pt in PtMeasureTypes:
    for season in Seasons:
        ...
        df.to_csv('pt_data/%s_%s.csv' % (season, pt) , sep = ',', index = False)
